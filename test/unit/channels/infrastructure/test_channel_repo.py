...
from src.modules.core.database import engine, Base, get_async_session
from src.modules.channels.infrastructure.persistence.sqlal_channel_repo import (
...
class TestChannelRepo:
    ...
    async def url_seed(self, db_session, channel_seed):
        url = URLModel(
            public_id=str(uuid.uuid4()),
            channel_id=channel_seed.public_id,
    ...
    async def test_add_successfully(self, repo, db_session):
        ...
        assert saved_channel.public_id == channel_id.value
    ...
    async def test_get_by_id_successfully(self, repo, channel_entity, channel_seed):
        ...
        assert result.id.value == channel_seed.public_id
    ...
    async def test_exists_by_id(self, repo, channel_entity):
        ...
        assert await repo.exists_by_id(ID.generate()) is False
    ...
    async def test_update_name_successfully(self, repo, db_session, channel_entity):
        ...
        result = await db_session.execute(
            select(ChannelModel).where(
                ChannelModel.public_id == channel_entity.id.value
            )
        )
    ...
    async def test_delete_successfully(self, repo, db_session, channel_entity):
        ...
        result = await db_session.execute(
            select(exists().where(ChannelModel.public_id == channel_entity.id.value))
        )
    ...
    async def test_search_all_channels(self, repo, channel_seed, second_channel_seed):
        ...
        assert channel_seed.public_id in ids
        assert second_channel_seed.public_id in ids

    async def test_search_with_exact_filter(self, repo, channel_seed):
        result = await repo.search(filters={"country_code": "US"})

        assert len(result) == 1
        assert result[0].id.value == channel_seed.public_id
    ...
    async def test_search_with_id_filter(self, repo, channel_seed):
        result = await repo.search(filters={"id": channel_seed.public_id})
    ...
    async def test_add_new_url_successfully(self, repo, db_session, channel_entity):
        ...
        result = await db_session.execute(
            select(URLModel).where(URLModel.public_id == url.id.value)
        )
        saved_url = result.scalar_one()

        assert saved_url.public_id == url.id.value
    ...
    async def test_add_new_url_multiple_urls(self, repo, db_session, channel_entity):
        url1 = URLEntity.create(url="https://www.cnn.com")
        url2 = URLEntity.create(url="https://edition.cnn.com")
    ...
    async def test_add_new_url_for_non_existence_channel(self, repo):
        url1 = URLEntity.create(url="https://www.cnn.com")

        with pytest.raises(ChannelNotFoundError):
            await repo.add_new_url(ID.generate(), url1)

    async def test_remove_url_successfully(self, repo, db_session, url_seed):
        await repo.remove_url(ID(url_seed.public_id))

        result = await db_session.execute(
            select(exists().where(URLModel.public_id == url_seed.public_id))
        )
        assert result.scalar() is False
    ...
    async def test_get_urls_successfully(self, repo, channel_entity, url_seed):
        ...
        assert len(result) == 1
        assert result[0].id.value == url_seed.public_id
